...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    const listaSpecs = formData.especializacion.split(",").map(s => s.trim()).filter(s => s !== "");
    if (listaSpecs.length === 0) { setError('Escriba al menos una especialidad'); return; }
    try {
      setIsLoading(true);
      setModalState({ isOpen: true, status: 'loading', message: 'Procesando registro...' });
      const userPayload = {
        ci: Number(formData.C_I), nombre: String(formData.nombre), apellido: String(formData.apellido), email: String(formData.correo), password: String(formData.password), telefono: Number(formData.telefono), ficha: Number(formData.ficha), rol: String(formData.rol), extension: Number(formData.extension),
      };
      ...
      if (formData.numero_maquina) {
        try { await axios.post(`${base}/machines`, { id_user: Number(userId), nro_maquina: Number(formData.numero_maquina) }); } catch (e) { console.warn(e); }
      }
      for (const nombre of listaSpecs) {
        try {
          ...
          if (specId) await axios.post(`${base}/specialization_users`, { id_user: Number(userId), id_specia: Number(specId) });
        } catch (e) { console.warn(e); }
      }
      setModalState({ isOpen: true, status: 'success', message: '¡Registro completado con éxito!' });
    } catch (err) {
  ...
  const handleModalClose = () => {
    ...
    if (wasSuccess) navigate('/', { replace: true });
  };

  return (
    // CAMBIO: bg-blue-600 -> bg-slate-50
    <div className={`${darkMode ? "bg-gray-900" : "bg-slate-50"} min-h-screen flex items-center justify-center p-2 sm:p-4 md:p-8 transition-colors duration-300`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 sm:gap-y-4 text-left">
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Nombre</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Apellido</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Correo</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Contraseña</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Cédula</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Ficha</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Rol</label>
                  <select name="rol" value={formData.rol} onChange={(e) => handleInputChange(e, setFormData)} className={`${darkMode ? "bg-gray-800 border-gray-700 text-white" : "bg-gray-50 border-gray-300"} w-full p-2.5 text-sm border rounded-xl outline-none focus:ring-2 focus:ring-blue-600 transition-all`} required>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Especialidades</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Nro Máquina</label>
              ...
              <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-black uppercase text-blue-500 ml-1">Teléfono</label>
            ...
            <button type="submit" disabled={isLoading} className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-black hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-50 mt-6 uppercase tracking-widest text-sm">
